    ...
    for i in concerned_folders:
        if not os.path.exists(f'{context.op_config[i]}/{context.op_config["observation_date"]}'):
            ...
            if i == "reporting_data_path":
                os.mkdir(f'{context.op_config["reporting_data_path"]}/{context.op_config["observation_date"]}/controle_mention_adresse')
                os.mkdir(f'{context.op_config["reporting_data_path"]}/{context.op_config["observation_date"]}/controle_mention_adresse/consolidation')
                os.mkdir(f'{context.op_config["reporting_data_path"]}/{context.op_config["observation_date"]}/docelec')
                os.mkdir(f'{context.op_config["reporting_data_path"]}/{context.op_config["observation_date"]}/open_access')
                os.mkdir(f'{context.op_config["reporting_data_path"]}/{context.op_config["observation_date"]}/open_access/consolidation')
...
def get_publis_all_with_affiliations_data(context,df_reference_data,df_affiliations):
    ...
    publis_all_with_affiliations_data.to_csv(f'{context.op_config["reporting_data_path"]}/{context.op_config["observation_date"]}/open_access/publis_all_with_affiliations_data.csv',index = False,encoding='utf8')
...
@job(
    metadata={
        "notes": MetadataValue.text("Création de l'architecture de dossiers nécessaire")
    }
)
def a_1_create_subfolders():
...
def prod_bso_publis_scopus():
    return [a_1_create_subfolders,a_2_core_unpaywall]
